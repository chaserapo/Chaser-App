...
const CONTENT: Record<string, { title: string; effective: string; body: { heading?: string; text: string }[] }> = {
  privacy: {
    ...
    body: [
      ...
      { heading: "3. Where your data is stored", text: "Your data is stored in a Supabase-hosted PostgreSQL database. Each farm business's data is protected by row-level security (RLS), so team members from other businesses cannot access your records. Our backend, hosted on Railway, connects to this database to handle account deletion and to deliver the notifications described above. Supabase's data centres may be outside your country — by using Chaser you consent to this cross-border storage." },

      { heading: "4. Who can see your data", text: "Only you and the team members you have invited (and accepted) into your farm business can read your farm data. Chaser support staff can, with your consent, access your data to investigate a support request. Separately, our backend runs limited automated processes — such as checking whether a weather or maintenance condition you've asked to be alerted about has been met — that read across businesses only for that narrow purpose; this is an automated process, not manual access by a person. Sub-processors are: Supabase (database, authentication), Railway (backend hosting), weather data providers including Open-Meteo (coordinates only, no account), Resend and Emergent (transactional email delivery), Expo (push notification delivery), and Apple (App Store subscription billing)." },

      { heading: "5. Location, camera & other permissions", text: "Chaser only reads your device GPS when you explicitly capture weather, record a paddock boundary, or record a spray job. Location is not tracked in the background. If you deny the location permission, Chaser falls back to a default regional weather lookup and you can still enter data manually. Chaser only accesses your camera or photo library when you choose to attach a photo to a fault report or spray record — photos are never accessed in the background." },
